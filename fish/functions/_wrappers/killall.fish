if in-path killall
  ...
  function killall --description 'Kill processes by name'
    ...
    function _pgrep; command ps -s | command grep -w $argv[1] | awk '{print $1}'; end
    function _pgrep-q; command ps -p $argv[1] >/dev/null; end
    function _kill; /bin/kill --force $argv; end
