function ls-vscode -d "List all Visual Studio Code workspaces"
    ...
    for file in $files
        ...
        if path is $path
            echo -s (set_color brgreen) 200 (set_color green) \t (my-prompt-pwd -Z $path) (set_color normal)
        else
            echo -s (set_color brred) 404 (set_color red) \t \e"[9m" (my-prompt-pwd -Z $path) (set_color normal)
        end
    end
end
