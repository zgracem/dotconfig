...
complete -c cygpath -s l -l long-name -d 'Print Windows long form of NAMEs (with -w, -m only)' -n '__fish_seen_argument -s w -l windows; or __fish_seen_argument -s m -l mixed'
complete -c cygpath -s p -l path -d 'NAME is a PATH list (i.e. , "/bin:/usr/bin")'
complete -c cygpath -s U -l proc-cygdrive -d 'Emit /proc/cygdrive path instead of /cygdrive prefix'
complete -c cygpath -s s -l short-name -d 'Print DOS (short) form of NAMEs (with -w, -m only)' -n '__fish_seen_argument -s w -l windows; or __fish_seen_argument -s m -l mixed'
complete -c cygpath -s C -l codepage -r -a 'ANSI OEM UTF8' -d 'Print DOS/Windows/mixed pathname in CODEPAGE'
complete -c cygpath -s A -l allusers -d 'Use "All Users" instead of current user (for -D, -O, -P)' -n '__fish_seen_argument -s D -l desktop; or __fish_seen_argument -s O -l mydocs; or __fish_seen_argument -s P -l smprograms'
...
complete -c cygpath -s o -l option -d 'Read options from FILE as well (for use with -f)' -n '__fish_seen_argument -s f -l file'
